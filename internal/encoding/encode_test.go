...
func TestDecodeKey_Malformed(t *testing.T) {
	// 缺少 tag 字节
	_, _, err := DecodeKey([]byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09})
	if err != ErrMalformedKey {
		t.Errorf("expected ErrMalformedKey for wrong tag, got %v", err)
	}
	// 截断的 TableHash
	_, _, err = DecodeKey([]byte{0x00, 0x01, 0x02, 0x03})
	if err != ErrMalformedKey {
		t.Errorf("expected ErrMalformedKey for truncated TableHash, got %v", err)
	}
}
...
func TestDecodeIndexKey_Malformed(t *testing.T) {
	tests := []struct {
		...
		{"empty", []byte{}},
		{"missing prefix", []byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09}},
		{"truncated after prefix", []byte{0xFF}},
		{"truncated TableHash", []byte{0xFF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07}},
		{"no fieldNum", []byte{0xFF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}},
		{"no valueLen", []byte{0xFF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x03}},
		{"valueLen overruns", []byte{0xFF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x03, 0x0A, 'x'}}, // 声明 10 字节只有 1 字节
	}
