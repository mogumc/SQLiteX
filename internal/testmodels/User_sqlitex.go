...
func NewUserStore(db *sqlitex.DB) UserStore {
	return &userStore{db: db, tableID: 12299085994647318028}
}
...
func (q *UserQuery) execIndexed() ([]*User, error) {
	...
	prefix := encoding.EncodeIndexPrefix(12299085994647318028, 3, fieldValue)
	...
	for iter.Next() {
		pkBytes := iter.Value()
		dataKey := encoding.EncodeKey(12299085994647318028, pkBytes)
...
func (q *UserQuery) execFullScan() ([]*User, error) {
	prefix := encoding.EncodeKey(12299085994647318028, nil)
	iter := q.db.Iterate(prefix)
	if iter == nil { return nil, nil }
	defer iter.Close()
	if len(q.lastKey) > 0 { iter.SeekLT(q.lastKey); if iter.Valid() { iter.Next() } }
...
func NewSessionStore(db *sqlitex.DB) SessionStore {
	return &sessionStore{db: db, tableID: 17504996543229074857}
}
...
func (q *SessionQuery) execIndexed() ([]*Session, error) {
	...
	var fieldValue []byte
	fieldValue = []byte(val.(string))
	prefix := encoding.EncodeIndexPrefix(17504996543229074857, 3, fieldValue)
	iter := q.db.Iterate(prefix)
	if iter == nil { return nil, nil }
	defer iter.Close()
	if len(q.lastKey) > 0 { iter.SeekLT(q.lastKey); if iter.Valid() { iter.Next() } }
	...
	for iter.Next() {
		pkBytes := iter.Value()
		dataKey := encoding.EncodeKey(17504996543229074857, pkBytes)
		value, err := q.db.Get(dataKey)
		if err != nil || value == nil { continue }
		m, expiresAt, err := DeserializeSessionMeta(value)
		if err != nil { continue }
		if expiresAt > 0 && time.Now().UnixNano() > expiresAt {
			deleteSessionWithIndexes(q.db, 17504996543229074857, dataKey, m) // 惰性删除 + 索引清理
			continue
		}
		if len(q.where) > 1 && !q.matchWhereTail(m) { continue }
		results = append(results, m)
...
func (q *SessionQuery) execFullScan() ([]*Session, error) {
	prefix := encoding.EncodeKey(17504996543229074857, nil)
	iter := q.db.Iterate(prefix)
	if iter == nil { return nil, nil }
	defer iter.Close()
	if len(q.lastKey) > 0 { iter.SeekLT(q.lastKey); if iter.Valid() { iter.Next() } }
	...
	for iter.Next() {
		...
		if err != nil { continue }
		if expiresAt > 0 && time.Now().UnixNano() > expiresAt {
			deleteSessionWithIndexes(q.db, 17504996543229074857, key, m) // 惰性删除 + 索引清理
			continue
		}
		if q.matchWhere(m) {
			results = append(results, m)
